...
A repo-agnostic starter kit for building Power Platform model-driven apps from VS Code using the Power Platform CLI and Dataverse Web API. Clone it into any project to get a guided, scripted path from idea to working solution ╬ô├ç├╢ without manual portal clicks.
...
Three entry points ╬ô├ç├╢ all leading to the same planning and build sequence:
...
```text
Discovery ╬ô├Ñ├å Spec Kit ╬ô├Ñ├å Demo script ╬ô├Ñ├å Dataverse schema ╬ô├Ñ├å App experience ╬ô├Ñ├å Solution export ╬ô├Ñ├å Git ╬ô├Ñ├å Import
```
...
Prompts for environment URL, Azure tenant, publisher prefix, and solution names. Saves a local `.env.ps1` ╬ô├ç├╢ never committed (protected by `.gitignore`).
...
> [!IMPORTANT]
> Do not run build scripts until your planning files are complete and reviewed (`spec.md`, `plan.md`, and `tasks.md` in root or under `specs/<scenario-slug>/`). This is a hard gate ╬ô├ç├╢ building before planning creates rework.
...
After each script: check that the failed count is zero before running the next. All scripts are idempotent ╬ô├ç├╢ safe to rerun after fixing any failure.
...
```text
Terminal wizard               ╬ô├Ñ├å spec.md, plan.md, tasks.md ╬ô├Ñ├å demo-walkthrough.md + demo-talk-track.md ╬ô├Ñ├å build scripts
Copilot Chat                  ╬ô├Ñ├å same planning artifacts     ╬ô├Ñ├å demo-walkthrough.md + demo-talk-track.md ╬ô├Ñ├å build scripts
Claude Code skill (available) ╬ô├Ñ├å guides the entire flow      ╬ô├Ñ├å inline help at each step
```
...
`git status` must show a clean working tree after commit. Never commit `.env.ps1` ╬ô├ç├╢ it contains tokens and is protected by `.gitignore`. Verify with `git status` before every commit.
  ...
  .claude/
    skills/
      power-platform-vscode-wizard/
        SKILL.md              ╬ô├ç├╢ Claude Code skill: wizard workflow, scripts, troubleshooting
  .github/
    copilot-instructions.md   ╬ô├ç├╢ Repo-wide Copilot Chat behavior and workflow guidance
    prompts/
      power-platform-demo-wizard.prompt.md ╬ô├ç├╢ Slash prompt for guided chat-based wizard
  .vscode/
    extensions.json           ╬ô├ç├╢ Recommended extensions (installs on first open)
  .gitignore                  ╬ô├ç├╢ Protects .env.ps1 (tokens/secrets) from accidental commits
  docs/
    onboarding.md             ╬ô├ç├╢ Step-by-step setup guide for new builders
    build-log.md              ╬ô├ç├╢ Log template for recording each build run
    wizard-contract-v1.md     ╬ô├ç├╢ Canonical workflow contract (discovery/planning/execution)
  payloads/                   ╬ô├ç├╢ Add table-*.json, columns-*.json, relationships-*.json here
  requirements/
    how-to-build-dynamics-model-driven-apps-in-vscode-with-copilot.md  ╬ô├ç├╢ Full implementation playbook
    how-to-build-dynamics-model-driven-apps-wizard.md                  ╬ô├ç├╢ Guided wizard and Spec Kit steps
  scripts/
    ci/
      test-docs-consistency.ps1  ╬ô├ç├╢ Guardrail: docs/prompt question-contract consistency
      test-script-order.ps1      ╬ô├ç├╢ Guardrail: docs/prompt script-order consistency
      test-script-smoke.ps1      ╬ô├ç├╢ Guardrail: reliability smoke checks (relationships/add-to-solution/view layout)
  scripts/
    bootstrap/
      00-prereq-check.ps1          ╬ô├ç├╢ Verify tools are installed (no changes made)
      01-install-skills.ps1        ╬ô├ç├╢ Copy Claude Code skills to ~/.claude/skills/
      05-start-wizard.ps1          ╬ô├ç├╢ Discovery questions ╬ô├Ñ├å Spec Kit starter files
      10-auth-connect.ps1          ╬ô├ç├╢ Sign in, configure PAC auth, save session
      20-build-tables.ps1          ╬ô├ç├╢ Create tables from payloads/table-*.json
      30-build-columns.ps1         ╬ô├ç├╢ Add columns from payloads/columns-*.json
      40-build-relationships.ps1   ╬ô├ç├╢ Create lookups from payloads/relationships-*.json
      50-add-to-solution.ps1       ╬ô├ç├╢ Add payload-referenced entities to target solution
      60-build-forms-views.ps1     ╬ô├ç├╢ Build payload-driven Starter Main Forms and Active views, then publish
      65-build-web-resources.ps1   ╬ô├ç├╢ Generate optional scenario-driven HTML report web resources and add to solution
      70-build-web-resources.ps1   ╬ô├ç├╢ Canonical optional reporting module entrypoint (wrapper)
      80-post-build-analysis.ps1   ╬ô├ç├╢ Post-build summary generation and optional README update/commit prompts
      wizard.profile.json          ╬ô├ç├╢ Project profile (required questions, modules, sequencing, conventions, gates)
      MIGRATION.md                 ╬ô├ç├╢ Upgrade guidance and compatibility expectations
```
...
| 401 on every API call | Token resource URL mismatch | Rerun `10-auth-connect.ps1` ╬ô├ç├╢ no trailing slash on environment URL |
| Token works then stops mid-run | Token expired (60╬ô├ç├┤90 min timeout) | Rerun `10-auth-connect.ps1` to refresh |
...
| Build script fails midway | Any error during execution | Scripts are idempotent ╬ô├ç├╢ fix issue and rerun the same script |
| `code` command not found | VS Code shell command not in PATH | Command palette ╬ô├Ñ├å "Shell Command: Install 'code' command in PATH", restart terminal |
...
| [docs/standard-dataverse-tables.md](docs/standard-dataverse-tables.md) | Reference for standard (out-of-box) vs. custom Dataverse tables ╬ô├ç├╢ helps you decide which to reuse and which to create |
