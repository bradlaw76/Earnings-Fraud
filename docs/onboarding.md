...
Additional required checkpoint:

- Confirm a model-driven review app exists (created or updated during the run) and contains the run artifacts users need to review quickly.
- Confirm the entry-point table and landing view (for example Cases and Active view) were explicitly selected and implemented.
...
## Step 11A: Inventory Artifacts and Sync to Selected Solution

This step prevents the most common drift issue: components exist in the environment but were never added to the intended solution.

Use this every run:

1. Build an artifact inventory from script output:
	- Tables, columns, relationships
	- Forms, views
	- App(s), sitemap updates
	- Web resources
2. Use the solution name selected during wizard setup.
3. Run add-to-solution synchronization for all inventory categories.
4. Publish after synchronization.
5. Produce a final membership report with:
	- Added successfully
	- Already in solution
	- Failed with reason
...
- All created/updated components are confirmed in the selected solution.
- Any exceptions are explicitly listed for manual follow-up.
...
## Step 14B: Test the Copilot Studio Agent (When Applicable)

If your scenario includes a Copilot Studio agent (generated by `06-demo-script-wizard.ps1`), complete these steps before the demo:

1. Open the published agent in Copilot Studio or the embedded browser experience.
2. Run each test prompt listed in the `[DRAFT] Copilot Studio Agent` section of `demo-walkthrough.md`.
3. Confirm the agent cites specific regulatory sections or knowledge source content (not generic answers).
4. Update the `[DRAFT]` marker to `[READY]` in both `demo-walkthrough.md` and `demo-talk-track.md`.
5. Check the setup checklist in the walkthrough — all boxes must be ticked before demo day.

For the SSA Earnings Integrity Advisor specifically:
- Knowledge source: `https://www.ssa.gov/OP_Home/cfr20/cfrdoc.htm`
- Verify the agent references 20 CFR Part 404, 416, or 498 when asked about earnings fraud topics.

Validation checkpoint:

- Agent responds to all five test prompts with regulation-grounded answers.
- Agent correctly distinguishes OASDI (Part 404) rules from SSI (Part 416) rules.
- No adjudication decisions are returned — guidance only.
